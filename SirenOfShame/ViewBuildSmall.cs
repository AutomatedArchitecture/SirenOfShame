...
namespace SirenOfShame
{
    public partial class ViewBuildSmall : UserControl
    {
        ...
        private static readonly ILog _log = MyLogManager.GetLogger(typeof(ViewBuildSmall));
        ...
        {
            return _settings.FindBuildDefinitionByName(BuildName);
        }

        private void EditRulesClick(object sender, EventArgs e)
        {
            BuildDefinitionSetting buildDefinitionSetting = GetActiveBuildDefinitionSetting();
            var location = PointToScreen(_editRules.Location);
            _buildMenu.Show(location.X + _editRules.Width, location.Y + _editRules.Height);
